...
class ParticleSystemReactNativeART extends Component {
  ...
  render() {
    const data = this.props.particles.map(p => `M${p.position[0]},${p.position[1]}a5,5 0 1,0 10,0a5,5 0 1,0 -10,0`)
...
class ParticleSystemReactNativeSvgWithRects extends Component {
  ...
  render() {
    return (
      <Svg height={HEIGHT} width={WIDTH}>
        {this.props.particles.map((p, i) => {
          return <Rect key={i} x={p.position[0]} y={p.position[1]} width={p.size} height={p.size} fill={p.color} />
        })}
      </Svg>
    )
  }
...
export { Particle, ParticleSystem, ParticleSystemReactNativeART, ParticleSystemReactNativeSvg, ParticleSystemReactNativeSvgWithRects }
